...
public abstract class GenTreeShadowless implements IWorldGenerator {
    @Override
    ...
    public void generate(IWorld world, IChunk chunk, Random rand) {
        ...
        for (int checkY = startY; checkY <= startY + thisHeight + crownRad + 1; checkY++) {
            for (int checkX = startX - crownRad - 1; checkX <= startX + crownRad + 1; checkX++) {
                ...
                if (world.getState(TileLayer.MAIN, checkX, checkY).getTile() == GameContent.TILE_LOG || world.getState(TileLayer.MAIN, checkX, checkY).getTile() == GameContent.TILE_LEAVES) {
                    spawn = false;
    ...
    public TileState getWood() {
        return ArboriaTiles.tileSunnyWood.getDefState();
    }

    public TileState getLeaves() {
        return ArboriaTiles.tileSunnyLeaves.getDefState();
    }
